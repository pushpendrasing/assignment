...
object assignment {
  ...
  */
   ...
   val  skipable_first_row = Districtdf.first()
    ...
    val finalDf = spark.sqlContext.sql("SELECT * FROM Districtdf2 CROSS JOIN WaterQualitydf2 on Districtdf2.Name_of_the_State/Union_territory_and_Districts == WaterQualitydf2.State_Name")
