...
object MobileDashboard {
  ...
  */
   ...
   val  skipable_first_row = Districtdf.first()
    ...
    val Districtdf2:DataFrame = Districtdf1.select((Districtdf1.col("_c0")).as("State_Code").cast("int"),(Districtdf1.col("_c1")as("District_Code")).cast("int"),(Districtdf1.col("_c2")as("State_Name")).cast("String"))
  ...
  val finalDF =Districtdf2.join(WaterQualitydf2,"State_Name")//val finalDf = spark.sqlContext.sql("SELECT * FROM Districtdf2 CROSS JOIN WaterQualitydf2 on Districtdf2.Name_of_the_State/Union_territory_and_Districts == WaterQualitydf2.State_Name")
